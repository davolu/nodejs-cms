...
import { ACTION_MAP, restRequest, runCustomAction } from '@/lib/actions'
...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  // Named action on a custom connector: "custom:<connectorId>:<actionId>"
  if (params.id.startsWith('custom:')) {
    const [, cid, aid] = params.id.split(':')
    const c = await resolveConnector(cid)
    const action = c?.actions?.find((a) => a.id === aid)
    if (!c || !action) return NextResponse.json({ error: 'Unknown action' }, { status: 404 })
    if (c.auth !== 'rest' && !(await connectorConnected(cid))) return NextResponse.json({ error: `${c.name} is not connected` }, { status: 400 })
    try { return NextResponse.json({ ok: true, result: await runCustomAction(c, action, input) }) }
    catch (e: any) { return NextResponse.json({ ok: false, error: String(e?.message || e).slice(0, 200) }, { status: 502 }) }
  }

  // Generic REST connector request: "rest:<connectorId>"
